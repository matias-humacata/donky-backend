...
const { cambiarEstado } = require('../services/turnoStateMachine');
const {
  parseTimeToMinutes,
  getMinutesOfDay,
  dateOnly,
 ...
 */
async function listTurnosPendientes(req, res) {
  try {
 ...
 * GET /api/turnos/all
 */
async function listAllTurnos(req, res) {
  try {
    const turnos = await Turno.find({})
      .populate('cliente')
      .populate('vehiculo')
 ...
 */
async function aprobarTurno(req, res) {
  try {
    const turno = await Turno.findById(req.params.id);
 ...
 */
async function rechazarTurno(req, res) {
  try {
    const turno = await Turno.findById(req.params.id);
    if (!turno) return res.status(404).json({ error: 'Turno no encontrado' });
